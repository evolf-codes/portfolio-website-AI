      ...
      {WORK_PROJECTS.map((project, index) => (
        <article id={project.slug} key={project.slug} className="work-showcase__item scroll-mt-24">
          <div className="work-showcase__copy">
            <p className="type-eyebrow">{String(index + 1).padStart(2, "0")} · {project.discipline}</p>
            ...
            <div className="mt-6 flex flex-wrap gap-5 text-sm">
              ...
              <a
                className="text-link"
                href={getProjectSourceUrl(project)}
                target="_blank"
                rel="noreferrer"
              >
                Project files
          ...
          <Link href={`/work/${project.slug}`} className="result-link overflow-hidden rounded-xl border border-[var(--border)] bg-white">
            <img src={project.resultImageSrc} alt={project.resultImageAlt} className="h-full w-full object-contain" loading={index < 2 ? "eager" : "lazy"} />
          </Link>
        </article>
      ))}
    </div>
